...
// 📺 Gade piblisite pou fè pyès (+10 🪙) — logique + limite jounalye nan ads.js
function updateRewardAdUI() {
    const box = $("reward-ad");
    if (!box || !window.NS4Ads) return;
    if (NS4Ads.isPro() || !NS4Ads.inApp()) { box.classList.add("hidden"); return; }   // PRO / navigatè: pa gen kat la
    box.classList.remove("hidden");
    const left = NS4Ads.coinAdRemaining();
    setText("reward-ad-left", left > 0 ? `Rete ${left} piblisite jodi a` : "Tounen demen pou plis!");
    const btn = $("reward-ad-btn");
    if (btn) btn.disabled = left <= 0;
}

function watchAdForCoins() {
    if (!window.NS4Ads) return;
    const btn = $("reward-ad-btn");
    if (btn) btn.disabled = true;
    NS4Ads.watchForCoins().then(() => {
        updateRewardAdUI();
...
window.addEventListener("load", updateRewardAdUI);
