...
/* ========= 7. PIBLISITE =========
   Tout passe par ads.js (NS4Ads): bannière (FREE seulement), interstitiel aux
   écrans de résultat (jamais pendant les questions). Compteur + délai partagés
   entre toutes les pages (localStorage), PRO = interstitiels rares.
   En navigateur (hors app) + ADS.testMode: carré "Piblisite (tès)". */
const Ads = {
  init() {
    if (!ADS.enabled) return;
    if (window.NS4Ads) window.NS4Ads.applyBanner();
    if (ADS.testMode && !(window.NS4Ads && window.NS4Ads.inApp()) && !(window.NS4Ads && window.NS4Ads.isPro())) {
      const ph = $("ad-banner");
      if (ph) ph.hidden = false;
    }
  },
  refresh() { this.init(); if (window.NS4Ads && window.NS4Ads.isPro()) this.hideBanner(); },
  hideBanner() {
    const ph = $("ad-banner");
    if (ph) ph.hidden = true;
    if (window.NS4Ads) window.NS4Ads.hideBanner();
  },
  maybeInterstitial() {
    if (ADS.enabled && window.NS4Ads) window.NS4Ads.maybeInterstitial("quiz");
  }
};
...
window.NS4_QUIZ_STATE = state;   // lu par nav.js (bouton retour Android)
